...
export namespace Core3WebApi_Controllers_Client {
	export class Statistics {
		 ...
		 */
		getDistribution(): Promise<Response> {
			return fetch(this.baseUri + 'api/Statistics/distribution', { method: 'get' });
		}
	}
...
export namespace DemoWebApi_Controllers_Client {
	export class Entities {
		...
		updatePerson(person: DemoWebApi_DemoData_Client.Person): Promise<Response> {
			return fetch(this.baseUri + 'api/Entities/updatePerson', { method: 'put', headers: { 'Content-Type': 'application/json;charset=UTF-8' }, body: JSON.stringify(person) });
		}
	}
	...
	export class Home {
		 ...
		 */
		index(): Promise<Blob> {
			return fetch(this.baseUri + 'api/Home', { method: 'get' }).then(d => d.blob());
		}
	}
